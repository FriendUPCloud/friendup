...
compile:
	@echo "Making default compilation with debug."
	make -C core DEBUG=1 WEBSOCKETS_THREADS=$(WEBSOCKETS_THREADS) USE_SELECT=$(USE_SELECT) NO_VALGRIND=$(NO_VALGRIND) CYGWIN_BUILD=$(CYGWIN_BUILD) USE_SSH_THREADS_LIB=$(USE_SSH_THREADS_LIB) LOG_TIMESTAMP=$(LOG_TIMESTAMP) OPENSSL_INTERNAL=$(OPENSSL_INTERNAL) ENABLE_SSH=$(ENABLE_SSH) COMMUNICATION_SERVICE=${COMMUNICATION_SERVICE} COMMUNICATION_REM_SERVICE=${COMMUNICATION_REM_SERVICE}
...
release:
	...
	make -C core DEBUG=0 release WEBSOCKETS_THREADS=$(WEBSOCKETS_THREADS) USE_SELECT=$(USE_SELECT) NO_VALGRIND=$(NO_VALGRIND) CYGWIN_BUILD=$(CYGWIN_BUILD) USE_SSH_THREADS_LIB=$(USE_SSH_THREADS_LIB) OPENSSL_INTERNAL=$(OPENSSL_INTERNAL) ENABLE_SSH=$(ENABLE_SSH) COMMUNICATION_SERVICE=${COMMUNICATION_SERVICE} COMMUNICATION_REM_SERVICE=${COMMUNICATION_REM_SERVICE}
