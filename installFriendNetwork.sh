...
$SUDO sed -i -- "s@friendcore_host@$friendCoreDomain@g" "$destPath"/config.js
$SUDO sed -i -- "s@stun_url.com@$turnAddress@g" "$destPath"/config.js
$SUDO sed -i -- "s@turn_url.com@$turnAddress@g" "$destPath"/config.js
$SUDO sed -i -- "s@turn_username@$turnUser@g" "$destPath"/config.js
$SUDO sed -i -- "s@turn_password@$turnPass@g" "$destPath"/config.js
$SUDO sed -i -- "s@Do not edit this file!@This file can be edited@g" "$destPath"/config.js
if [ "$TLS" -eq "1" ]; then
    $SUDO sed -i -- "s@//keyPath : 'path_to_key.pem'@keyPath : '$FRIEND_BUILD/cfg/crt/key.pem'@g" "$destPath"/config.js
    $SUDO sed -i -- "s@//certPath : 'path_to_certificate.pem'@certPath : '$FRIEND_BUILD/cfg/crt/certificate.pem'@g" "$destPath"/config.js
fi
