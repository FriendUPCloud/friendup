...
/* This file implements US-280.
   It looks up Liberator SAML groups, compares them with existing user Liberator applications,
   adds the apps when necessary, removes apps that the user should not have and adds/removes
   dock icons. It also creates new Guacamole user for a new Friend user.

   Corner cases:
   When an existing app path is modified then it will be added (because technically it is a new app)
   and a user will have yet another icon in the dock. This case should be handled manually by the admin
   by meddling in the database and removing old apps by hand.

   Better approach:
   When an app is updated all old configuration should remain. It should be added as a new app and the old
   app entry should have dummy/non-existing SAML groups set so that it will be automatically removed.
...
debug("User data".print_r($friend_user_data,TRUE));
    ...
    //step 1 - check if the key to look for in SAML data containing user groups is set in the DB
    if ($rs = $friend_db->fetchObject("SELECT Data FROM FSetting WHERE FSetting.Type = 'mitra' AND FSetting.Key = 'admin'")){
        $mitra_admin_settings = json_decode($rs->Data);
        if (array_key_exists('saml_appenforce', $mitra_admin_settings)){

            $user_group_membership_string = join( $saml_data['user'][$mitra_admin_settings->saml_appenforce], '::');

            //step 2 - get a list of apps that have SAML access groups set
            if ($app_set = $friend_db->FetchObjects("SELECT ID as id, Data as data FROM FSetting WHERE FSetting.Type = 'mitraapp' AND FSetting.Data LIKE '%saml_accessgroups%'")){

                //step 3 - build a list of Liberator apps that the user should have access to
                $desired_apps = array();
                $undesired_apps = array();
                foreach($app_set as $app){
                    $app_settings = json_decode($app->data);
                    $app_groups = explode(',', $app_settings->saml_accessgroups); //groups are separated by commas in the DB JSON entry
                    if (is_user_group_in_app_group($user_group_membership_string, $app_groups)){
                        $desired_apps[$app->id] = $app_settings;
                        $desired_apps[$app->id]->present = FALSE; //assume that the user does not have them configured
                    } else {
                ...
                $rs = $friend_db->FetchRow("SELECT Data FROM FSetting s WHERE s.UserID = '-1' AND s.Type = 'mitra' AND s.Key = 'database';");
                $settings = json_decode($rs['Data']);

                $guacamole_db = new SqlDatabase();
                if ($guacamole_db->open($settings->host,$settings->username,$settings->password)){
                    if ($guacamole_db->SelectDatabase( $settings->database)){

                        //step 5 - check if the particular user has the required apps already set in guacamole (and if the user exists at all)
                        $user_pattern = 'mitra\_'.$guacamole_db->EscapeString($friend_user_data['username'], TRUE/*strict*/).'\_%'; //FIXME use prepared statements

                        $query = "SELECT COUNT(*) AS c FROM guacamole_user WHERE username LIKE 'mitra_".$guacamole_db->EscapeString($friend_user_data['username'])."'";
                        $user_entry = $guacamole_db->fetchObjects($query);

                        if ($user_entry[0]->c > 0){ //user exists
                            debug("User exists in guacamole_user");

                            $query = "SELECT connection_id, connection_name FROM guacamole_connection WHERE connection_name LIKE '".$user_pattern."' AND protocol = 'rdp'";
                            $user_connections = $guacamole_db->fetchObjects($query);

                            $application_ids_to_remove = array();
                            $application_ids_to_add = array();

                            if ($user_connections){ //user has any connections
                                ...
                                add_apps($guacamole_db, $friend_db, $desired_apps, $application_ids_to_add, $friend_user_data);

                                //step 10 - remove apps from Liberator and dock that the user has but should NOT have
                                remove_apps($guacamole_db, $friend_db, $undesired_apps, $application_ids_to_remove, $friend_user_data);

                            } else { //user has no RDP connections, it probably does not exist
                        ...
                        } else { //user does not exist - create connections
                            debug("adding new user (path 2)");

                            //delete any previous connections that may exist - this could only happen if the user has been
                            //deleted in Friend and then logged in again via SAML with the same credentials
                            $deletequery = "DELETE FROM guacamole_connection WHERE connection_name LIKE '".$user_pattern."' AND protocol = 'rdp'";
...
function create_guacamole_account($guacamole_db, $friend_db, $friend_user_data){
    ...
    $insertquery = "INSERT INTO guacamole_user (username, password_salt, password_hash,password_date) VALUES ('".$guacamole_db->EscapeString($guacamole_username)."', @salt, UNHEX(SHA2(CONCAT('".$guacamole_db->EscapeString($guacamole_password)."', HEX(@salt)), 256)), NOW() );";
    ...
    //FIXME: find out if all those permissions are necessary (as connections are configured by poking in the database anyway)
    ...
    $guacamole_db->Query($query); //FIXME: find out if setting the READ permission twice is necessary
    debug($query);
    ...
    debug($query);
    $friend_db->Query( $clean_query ); // yes, we need to clean in case somebody gave that user manual access sometime in the past.
    $friend_db->Query( $query ); //yes - the password in the DB is "enclosed within quotes"
}
...
function add_apps($guacamole_db, $friend_db, $apps, $app_ids_to_add, $friend_user_data){
    ...
    foreach ($app_ids_to_add as $app_id){
        ...
        $insert_connection_query = "INSERT INTO guacamole_connection (connection_name, parent_id, protocol) VALUES ('".$guacamole_db->EscapeString($guacamole_connection_name)."', 1, 'rdp');";
        ...
        $guacamole_user_id = $guacamole_db->fetchObject("SELECT user_id AS id FROM guacamole_user WHERE username = '".$guacamole_db->EscapeString($guacamole_username, FALSE/*not strict*/)."';")->id;

        //step 3 - add permissions for the new connection
        $add_permission_query = "INSERT INTO guacamole_connection_permission (user_id, connection_id, permission) VALUES (".$guacamole_user_id.",".$new_connection_id.",'READ')";
...
function remove_apps($guacamole_db, $friend_db, $apps, $app_ids_to_remove, $friend_user_data){
    ...
    foreach ($app_ids_to_remove as $app_id){
        ...
        $query = "DELETE FROM DockItem WHERE UserID =".$friend_user_data['userid']." AND Application = '".$friend_db->EscapeString('Mitra '.$apps[$app_id]->remoteapplicationprogram)."'";
         debug($query);
...
function guacamole_add_connection_parameter($guacamole_db, $connection_id, $name, $value){
    $query = "INSERT INTO guacamole_connection_parameter (connection_id, parameter_name, parameter_value) VALUES (".$connection_id.",'".$guacamole_db->EscapeString($name)."','".$guacamole_db->EscapeString($value)."');";
    $guacamole_db->Query($query);
}
