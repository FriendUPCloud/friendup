<?php
	...
	/*
		PHP file that will be include by simplesamlscript after the user has successfully authenticated him/herself.
	*/
	...
	if( !function_exists('authoriseFriendSAMLUser') )
	{
		...
		function authoriseFriendSAMLUser( $uid, $usermail, $displayname, $samldata )
		{
			...
			if( !(isset($conf['DatabaseUser']['host'])&&isset($conf['DatabaseUser']['login'])&&isset($conf['DatabaseUser']['password'])&&isset($conf['DatabaseUser']['dbname'])&&isset($samlconf['Module']['samlendpoint'])) ) die('CORRUPT FRIEND INSTALL!');
			...
			$getuserquery = 'SELECT fu.* FROM FUser fu, FUserToGroup futg, FUserGroup fug WHERE fu.Name = \''. mysqli_real_escape_string($dbo->_link, $uid) .'\' AND fu.Password = \''. mysqli_real_escape_string($dbo->_link, '{S6}' . hash('sha256', generateSAMLUserPassword( $uid ) )  ) .'\' AND fu.ID = futg.UserID AND futg.UserGroupID = fug.ID AND fug.Name=\'User\' AND fug.Type=\'SAML\';';
			...
			else
			{
				...
				$groupquery = 'INSERT INTO `FUserToGroup` (`UserID`,`UserGroupID`) VALUES ('. intval( $udata->ID ) .', (SELECT `ID` FROM `FUserGroup` WHERE `Name`=\'User\' AND `Type`=\'Level\'));';
				$rs = $dbo->Query($groupquery);

				checkSAMLUserGroup( $dbo );
				
				//add user to SAML users group....
				$groupquery = 'INSERT INTO `FUserToGroup` (`UserID`,`UserGroupID`) VALUES ('. intval( $udata->ID ) .', (SELECT `ID` FROM `FUserGroup` WHERE `Name`=\'User\' AND `Type`=\'SAML\'));';
				$rs = $dbo->Query($groupquery);
...
$ret = <<<EOT
<html><head><title>login - pease wait</title></head><body>
<div id="result"></div>
<script type="text/javascript">
var loginTries = 0;
function showLoginError() { document.getElementById("result").innerHTML = "<h1>Error during login! Please try again!</h1>"; }
