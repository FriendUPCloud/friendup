...
if( $rows = $SqlDatabase->FetchObjects( $q = ( '
	SELECT * FROM FMailHeader WHERE
	    UserID=\'' . $User->ID . '\' 
	AND `Address`=\'' . mysql_real_escape_string( $args->args->account ) . '\' 
	AND ExternalMessageID IN ( ' . mysql_real_escape_string( $args->args->ids ) . ' )
	AND `Folder` = \'' . mysql_real_escape_string( $args->args->folder ) . '\'
	ORDER BY `Date` DESC
' ) ) )
{
