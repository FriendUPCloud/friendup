...
Http *HandleWebDav( void *lsb, Http *req, char *data, int len )
{
	...
	if( strncmp( req->http_RawRequestPath, WEBDAV_SHARE_PATH, WEBDAV_SHARE_PATH_LEN ) != 0 )
	{
		struct TagItem tags[] = {
	...
	if( strcmp( req->http_Method, "OPTIONS" ) == 0 )
	{
...
<p>This server could not verify that you are authorized to access the document requested.  Either you supplied the wrong credentials (e.g., bad password), or your browser doesn't understand how to supply the credentials required.</p><hr><address>Apache/2.4.25 (Debian) Server at 215.148.12.6 Port 80</address></body></html>" );
		...
		HttpWrite( resp, req->http_Socket );
		int rs;
		char buf[ 10000 ];
		int t=0;
	...
	FD_ZERO( &fds );
	FD_SET( req->http_Socket->fd, &fds );
	
	timeout.tv_sec = 5;//sock->s_Timeouts;
	timeout.tv_usec = 5000;//sock->s_Timeoutu;
	
	SocketSetBlocking( req->http_Socket, TRUE );
	
	int err = select( req->http_Socket->fd+1, &fds, NULL, NULL, &timeout );
	if( err <= 0 )
	{
	...
	{
		if( req->http_RespHeaders[ z ] != NULL )
		{
			DEBUG("Found entry: %s\n", HEADERS[ z ] );
		}
	}
...
#define FS_READ_BUFFER 400960
				...
				if( req->http_RespHeaders[ HTTP_HEADER_RANGE ] == NULL )
				{
					...
					while( ( dataread = actFS->FileRead( fp, dataBuffer, FS_READ_BUFFER ) ) != -1 )
					{
						...
						if( futbytes >= req->http_RangeMin && total <= req->http_RangeMax )
						{
							...
							if( futbytes >= req->http_RangeMin )
							{
								...
								offset = dataread - (futbytes - req->http_RangeMin);
				...
				//BufStringDelete( bs );
				ListStringJoin( ls );
				resp->http_Content = ls->ls_Data;
				resp->http_SizeOfContent = ls->ls_Size;
				ls->ls_Data = NULL;
	...
	else if( strcmp( req->http_Method, "HEAD" ) == 0 )
	{
		struct TagItem tags[] = {
			{ HTTP_HEADER_CONTENT_TYPE, (FULONG)  StringDuplicate( "text/xml" ) },
			{	HTTP_HEADER_CONNECTION, (FULONG)StringDuplicate( "close" ) },
	...
	else if( strcmp( req->http_Method, "PUT" ) == 0 )
	{
		...
		if( have == TRUE )
		{
			...
			if( fp != NULL )
			{
				int saveSize = req->http_SizeOfContent;
				...
				{
					writelen = actFS->FileWrite( fp, req->http_Content, saveSize );
				}
				else
				{
	...
	else if( strcmp( req->http_Method, "MOVE" ) == 0 )
	{
		/*
...
<d:href>opaquelocktoken:e71d4fae-5dec-22df-fea5-00a0c93bd5eb1</d:href> \
	...
	else if( strcmp( req->http_Method, "UNLOCK" ) == 0 )
	{
		struct TagItem tags[] = {
			{ HTTP_HEADER_CONTENT_TYPE, (FULONG)  StringDuplicate( "text/xml" ) },
			{	HTTP_HEADER_CONNECTION, (FULONG)StringDuplicate( "close" ) },
...
HTTP/1.1 204 No Content
	...
	else if( strcmp( req->http_Method, "PROPPATCH" ) == 0 )
	{
	/*
