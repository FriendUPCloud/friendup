...
Socket* SocketNewSSL( void *sb, unsigned short port, int type )
{
	Socket *sock = NULL;
	...
	if( type == SOCKET_TYPE_SERVER )
	{
...

		{
			...
			SocketSetBlockingSSL( sock, FALSE );
...
int SocketListenSSL( Socket *sock )
{
	if( sock == NULL )
	{
		return -1;
...
int SocketConnectClientSSL(const char *hostname, int port, int family __attribute__((unused)), int socktype)
{
...
int SocketConnectSSL( Socket* sock, const char *host )
{
	...
	if( ( n = SocketConnectClientSSL( host, sock->port, AF_UNSPEC, SOCK_STREAM ) ) < 0 )
	{
...

	{
		...
		SocketSetBlockingSSL( sock, TRUE );
...
Socket* SocketConnectHostSSL( void *sb, char *host, unsigned short port )
{
	Socket *sock = NULL;
	...
	if( ( fd = SocketConnectClientSSL( host, port, AF_UNSPEC, SOCK_STREAM ) ) < 0 )
	{
		FERROR("Cannot setup connection with : %s\n", host );
		return NULL;
	}
	...
	DEBUG("[SocketConnectHost] connect to host\n" );

	{
		...
		SocketSetBlockingSSL( sock, blocked );
...
int SocketSetBlockingSSL( Socket* sock, FBOOL block )
{
	int flags, s = 0;
...
inline Socket* SocketAcceptSSL( Socket* sock )
{
	...
	if( incoming != NULL )
	{
		...
		SocketSetBlockingSSL( incoming, FALSE );
...
inline Socket* SocketAcceptPairSSL( Socket* sock, struct AcceptPair *p )
{
	// Don't bother with non-listening sockets
	if( sock == NULL )
	{
...
inline int SocketReadSSL( Socket* sock, char* data, unsigned int length, unsigned int expectedLength )
{
	if( sock == NULL )
	{
...
void SocketAbortWriteSSL( Socket* sock )
{
	if( sock == NULL )
	{
		return;
