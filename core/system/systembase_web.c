								...
								else
								{
									...
									if( sqllib != NULL )
									{
										...
										sqllib->SNPrintF( 
											...
											"SELECT `AuthID` FROM `FUserApplication` WHERE `UserID` = '%lu' AND `ApplicationID` = ( SELECT ID from `FApplication` WHERE `Name` = '%s' AND `UserID` = '%ld' )",
										...
										snprintf( 
											tmp, sizeof(tmp), 
											"{\"response\":\"%d\",\"sessionid\":\"%s\",\"authid\":\"%s\"}",
