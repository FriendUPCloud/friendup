...
int MountFS( struct SystemBase *l, struct TagItem *tl )
{
	...
	if( usr == NULL )
	{
		ERROR("[ERROR]: Device with that name is already mounted (%s)\n", name);
		l->sl_Error = FSys_Error_DeviceAlreadyMounted;
		return FSys_Error_DeviceAlreadyMounted;
	}
	
	DEBUG("Mount Checking avaiable filesystem\n");
...
//	FHandler *efsys = NULL;
	...
	if( retFile != NULL )
	{
		...
		else
		{
			...
			DEBUG("Device added to end of the list\n");
		}
