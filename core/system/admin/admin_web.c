	...
	*/
	...
	else if( strcmp( urlpath[ 1 ], "restartws" ) == 0 )
	{
		if( loggedSession->us_User->u_IsAdmin == TRUE )
		{
			...
			if( ( l->fcm->fcm_WebSocket = WebSocketNew( l,  l->fcm->fcm_WSPort, l->fcm->fcm_WSSSLEnabled, 0, l->fcm->fcm_WSExtendedDebug, l->fcm->fcm_WSTimeout, l->fcm->fcm_WSPingPongInterval ) ) != NULL )
			{
