		...
		if( host != NULL )
		{
			...
			if( df != NULL )
			{
				...
				else
				{
					...
					snprintf( dictmsgbuf, sizeof(dictmsgbuf), ERROR_STRING_TEMPLATE, dictmsgbuf1 , DICT_SERVER_CONNECT_ERROR );
					HttpAddTextContent( response, dictmsgbuf );
				}
			...
			{
				char dictmsgbuf[ 256 ];
				snprintf( dictmsgbuf, sizeof(dictmsgbuf), ERROR_STRING_TEMPLATE, l->sl_Dictionary->d_Msg[DICT_CANNOT_CONVERT_MESSAGE] , DICT_CANNOT_CONVERT_MESSAGE );
				HttpAddTextContent( response, dictmsgbuf );
			}
			FFree( host );
		}
	...
	else if( strcmp( urlpath[ 1 ], "servermessage" ) == 0 )
	{
		...
		if( msg != NULL )
		{
			...
			else	//is admin
			{
				...
				{
					char dictmsgbuf[ 256 ];
					int size = snprintf( dictmsgbuf, sizeof(dictmsgbuf), ERROR_STRING_TEMPLATE, l->sl_Dictionary->d_Msg[DICT_ADMIN_RIGHT_REQUIRED] , DICT_ADMIN_RIGHT_REQUIRED );
	...
	*/
	...
	else if( strcmp( urlpath[ 1 ], "restartws" ) == 0 )
	{
		if( IS_SESSION_ADMIN( loggedSession ) )
		{
			...
			if( ( l->fcm->fcm_WebSocket = WebSocketNew( l,  l->fcm->fcm_WSPort, l->fcm->fcm_WSSSLEnabled, 0, l->fcm->fcm_WSExtendedDebug, l->fcm->fcm_WSTimeout, l->fcm->fcm_WSka_time, l->fcm->fcm_WSka_probes, l->fcm->fcm_WSka_interval, l->fcm->fcm_WorkspaceServiceTimeout, l->fcm->fcm_WorkspaceServiceSleepTime ) ) != NULL )
			{
