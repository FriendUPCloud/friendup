...
char *EncodeStringLenSHA256( char *data, int len )
{
	...
	char *hashTarget;  // only 64 chars, but we extend this to put 0 on the end of string;
	
	if( ( hashTarget = FMalloc( 128 ) ) != NULL )
	{
		Sha256Init( &ctx );
		Sha256Update( &ctx, (unsigned char *) data, len ); //&(usr->u_Password[4]), strlen( usr->u_Password )-4 );
...
char *EncodeStringSHA256( char *data )
{
	...
	{
		Sha256Init( &ctx );
		Sha256Update( &ctx, (unsigned char *) data, (unsigned int)strlen( data ) ); //&(usr->u_Password[4]), strlen( usr->u_Password )-4 );
		Sha256Final( &ctx, hash );
