...
Workspace = {
	...
	login: function( u, p, r, callback, ev )
	{
		...
		{
			// Check localStorage if Workspace data is set if not generate keys and store it
			
			var storage = ApplicationStorage.load( {
				applicationName : 'Workspace'
			} );
			
			if( /*1==1 || */!storage || !storage.privatekey || this.loginUsername != storage.username || this.loginPassword != storage.password )
			{
				var keys = Authenticate.getKeys( true, {
					seed : ( u + ':' + window.MD5( p ) )
				} );
			...
			*/
			...
			if( t.sessionId ) m.addVar( 'sessionid', t.sessionId );
				...
				if( json.result == '0' || hasSessionID || hasLoginID || json.result == 3 )
				{
					...
					l.onExecuted = function( e, d )
					{
						...
						if( e == 'ok' )
						{
							...
							i18nAddPath( 'locale/' + Workspace.locale + '.locale' );
						}
						else
						{
...
var vrCanvas, vrScene, vrCamera, vrCrosshair, vrRoom = {}, vrRenderer;
var vrControls, vrRaycaster, vrInitialized;
var vrLogo, vrRot = 0;
var vrRooms = [ vrRoom ]; // List of rooms
var currentRoom = 0;
var zPhase = 0.0, INTERSECTED = null;

function InitFriendVR()
{
	if( vrInitialized ) return;
	if( !document.body || document.body && !document.body.classList.contains( 'Inside' ) ) 
		return setTimeout( InitFriendVR, 5 );
	vrInitialized = true;
	...
	// Setup the scene
	vrCanvas = document.createElement( 'div' );
	document.body.appendChild( vrCanvas );
	vrCanvas.style.zIndex = 2147483646;
	...
	roomfloor.rotation.x = 90 / 180 * Math.PI;
	roomfloor.position.y = -0.7;
	roomfloor.receiveShadow = true;
	vrScene.add( roomfloor );
	
	// Ceiling of room
	var roomceiling = new THREE.Mesh(
		new THREE.PlaneGeometry( 6, 6, 1 ),
		new THREE.MeshBasicMaterial( { color: 0x505050, side: THREE.DoubleSide } )
	);
	roomceiling.rotation.x = 90 / 180 * Math.PI;
	roomceiling.position.y = 0.7;
	vrScene.add( roomceiling );
	...
	{
		vrLogo = obj;
		vrLogo.scale.set( 0.2, 0.2, 0.2 );
		vrLogo.position.x = 0;
		vrLogo.position.y = 0.7;
		vrLogo.position.z = -1;
		vrLogo.rotation.x = 90 / 180 * Math.PI;
		vrLogo.rotation.x = 90 / 180 * Math.PI;
		vrLogo.castShadow = true;
		vrLogo.receiveShadow = true;
		vrScene.add( vrLogo );
	} );
	...
	vrRenderer.shadowMapType = THREE.PCFSoftShadowMap;
	vrRenderer.setClearColor( 0x202050 );
	...
	vrCanvas.appendChild( vrRenderer.domElement );

	vrControls = new THREE.VRControls( vrCamera );
	vrEffect = new THREE.VREffect( vrRenderer );

	WEBVR.getVRDisplay( function ( display ) 
	{
		var btn = WEBVR.getButton( display, vrRenderer.domElement );
...
function FriendVRAnimate()
{
	vrEffect.requestAnimationFrame( FriendVRAnimate );
	FriendVRRender();
}

function FriendVRRender()
{
	// Find intersections (collisions with camera)
	vrRaycaster.setFromCamera( { x: 0, y: 0 }, vrCamera );
	var intersects = vrRaycaster.intersectObjects( vrRoom.children );
	...
	// Rotate the logo
	if( vrLogo )
		vrLogo.rotation.z = vrRot / 180 * Math.PI;
	vrRot += 0.5;

	// Update controls and render
	vrControls.update();
	vrEffect.render( vrScene, vrCamera );
}
